...
// ---------------------------------------------------------
const USUARIOS = {
  "profe":    { password: "clase2024", admin: true },
  "alumno1":  { password: "pass1234",  admin: false },
  "alumno2":  { password: "pass1234",  admin: false },
  "alumno3":  { password: "pass1234",  admin: false },
  "alumno4":  { password: "pass1234",  admin: false },
  "alumno5":  { password: "pass1234",  admin: false }
};

const STORAGE_KEY = "debate_usuario_actual";
const STORAGE_KEY_ADMIN = "debate_admin_actual";
...
async function procesarLogin(username, password) {
  ...
  if (usuarioHardcodeado && usuarioHardcodeado.password === password) {
    iniciarSesion(username, usuarioHardcodeado.admin === true);
    return;
  }
  ...
  try {
    ...
    if (doc.exists && doc.data().password === password) {
      iniciarSesion(username, doc.data().admin === true);
    } else {
...
function iniciarSesion(username, esAdmin) {
  currentUser = username;
  currentUserIsAdmin = esAdmin === true;
  localStorage.setItem(STORAGE_KEY, username);
  localStorage.setItem(STORAGE_KEY_ADMIN, String(currentUserIsAdmin));
  mostrarPantallaChat();
}
...
function mostrarPantallaChat() {
  ...
  document.getElementById("admin-badge").hidden = !currentUserIsAdmin;
...
function renderMensaje(id, data) {
  ...
  const botonBorrar = currentUserIsAdmin
